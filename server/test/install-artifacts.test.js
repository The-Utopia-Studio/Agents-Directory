...
test("A7 copy export is byte-for-byte the system artifact runtime executes", async (t) => {
  ...
  assert.equal(exported.artifactVersion, "biocraft-singleshot-v5");
  assert.equal(
    exported.filename,
    `A7-biocraft-singleshot-v5-${expectedDigest.slice(0, 7)}-SKILL.md`,
  );
  assert.match(
    exported.content,
    /^artifact_version: biocraft-singleshot-v5$/m,
  );
...
test("A7 ZIP contains the runtime folder and follows the evaluated version", async (t) => {
  ...
  assert.equal(
    ...
    `attachment; filename="A7-biocraft-singleshot-v5-${expectedDigest.slice(0, 7)}.zip"`,
  );
  ...
  assert.match(skill, /generated_sections_have_no_delimiter_separated_keyword_run/);
