...
export const executeApprovedA7V7Release = mutation({
  args: { releaseManifestDigest: v.string() },
  handler: async (ctx, args) => {
    const actor = await requireApprover(ctx);
    if (args.releaseManifestDigest !== A7_V7_RELEASE_MANIFEST_DIGEST) {
      throw new ConvexError({
        code: "RELEASE_MANIFEST_NOT_APPROVED",
        status: 403,
        message: "A7 v7 release manifest is altered or not approved",
      });
    }
    const now = Date.now();
    const agent = await oneAgent(ctx, "A7");
    const prior = await oneVersion(ctx, agent._id, A7_V7_RELEASE_SPEC.priorVersion);
    if (!prior || prior.artifact?.declaredDigest !== A7_V7_RELEASE_SPEC.priorArtifactSha256) {
      throw new Error("A7 v7 release requires the exact governed v6 artifact as its base");
    }
    const current = approvedVersionOf(agent as unknown as Record<string, unknown>);
    let candidate = await oneVersion(ctx, agent._id, A7_V7_RELEASE_SPEC.version.version);
    if (current !== prior._id && current !== candidate?._id) {
      throw new Error("A7 v7 release requires v6 or its exact v7 successor to be current");
    }
    if (candidate) {
      assertExact(
        "A7 v7",
        { state: candidate.state, artifact: candidate.artifact, basedOnVersionId: candidate.basedOnVersionId },
        { state: A7_V7_RELEASE_SPEC.version.state, artifact: A7_V7_RELEASE_SPEC.version.artifact, basedOnVersionId: prior._id },
      );
    } else {
      const candidateId = await ctx.db.insert("agentVersions", {
        agentId: agent._id,
        version: A7_V7_RELEASE_SPEC.version.version,
        state: A7_V7_RELEASE_SPEC.version.state,
        basedOnVersionId: prior._id,
    ...
    if (proposal) {
      assertExact(
        "A7 v7 proposal",
        { agentId: proposal.agentId, priorApprovedVersionId: proposal.priorApprovedVersionId, summary: proposal.summary },
        { agentId: agent._id, priorApprovedVersionId: prior._id, summary: A7_V7_RELEASE_SPEC.proposalSummary },
      );
    } else {
      const proposalId = await ctx.db.insert("proposals", {
        agentId: agent._id,
        ...
        candidateVersionId: candidate._id,
        status: "open",
        summary: A7_V7_RELEASE_SPEC.proposalSummary,
        createdBy: actor,
        createdAt: now,
      });
      proposal = (await ctx.db.get(proposalId))!;
    ...
    const runnerConfig = agent.executionContract.runnerConfig
      .filter((entry) => entry.key !== "artifactVersion")
      .concat({ key: "artifactVersion", value: A7_V7_RELEASE_SPEC.version.version });
    await ctx.db.patch(agent._id, {
      invocation: { type: "runtime", configRef: "server-owned:biocraft-singleshot-v7" },
      executionContract: { ...agent.executionContract, runnerConfig },
    });
    return {
      agentId: agent._id,
...
export const executeApprovedA7V9Release = mutation({
  args: { releaseManifestDigest: v.string() },
  handler: async (ctx, args) => {
    const actor = await requireApprover(ctx);
    if (args.releaseManifestDigest !== A7_V9_RELEASE_MANIFEST_DIGEST) {
      throw new ConvexError({
        code: "RELEASE_MANIFEST_NOT_APPROVED",
        status: 403,
        message: "A7 v9 release manifest is altered or not approved",
      });
    }
    const now = Date.now();
    const agent = await oneAgent(ctx, "A7");
    const prior = await oneVersion(ctx, agent._id, A7_V9_RELEASE_SPEC.priorVersion);
    if (!prior || prior.artifact?.declaredDigest !== A7_V9_RELEASE_SPEC.priorArtifactSha256) {
      throw new Error("A7 v9 release requires the exact governed v8 artifact as its base");
    }
    const current = approvedVersionOf(agent as unknown as Record<string, unknown>);
    let candidate = await oneVersion(ctx, agent._id, A7_V9_RELEASE_SPEC.version.version);
    if (current !== prior._id && current !== candidate?._id) {
      throw new Error("A7 v9 release requires v8 or its exact v9 successor to be current");
    }
    if (candidate) {
      assertExact(
        "A7 v9",
        { state: candidate.state, artifact: candidate.artifact, basedOnVersionId: candidate.basedOnVersionId },
        { state: A7_V9_RELEASE_SPEC.version.state, artifact: A7_V9_RELEASE_SPEC.version.artifact, basedOnVersionId: prior._id },
      );
    } else {
      const candidateId = await ctx.db.insert("agentVersions", {
        agentId: agent._id,
        version: A7_V9_RELEASE_SPEC.version.version,
        state: A7_V9_RELEASE_SPEC.version.state,
        basedOnVersionId: prior._id,
        artifact: A7_V9_RELEASE_SPEC.version.artifact,
        createdBy: actor,
        createdAt: now,
      } as any);
      candidate = (await ctx.db.get(candidateId))!;
    ...
    if (proposal) {
      assertExact(
        "A7 v9 proposal",
        { agentId: proposal.agentId, priorApprovedVersionId: proposal.priorApprovedVersionId, summary: proposal.summary },
        { agentId: agent._id, priorApprovedVersionId: prior._id, summary: A7_V9_RELEASE_SPEC.proposalSummary },
      );
    } else {
      const proposalId = await ctx.db.insert("proposals", {
        agentId: agent._id,
        ...
        candidateVersionId: candidate._id,
        status: "open",
        summary: A7_V9_RELEASE_SPEC.proposalSummary,
        createdBy: actor,
        createdAt: now,
      });
      proposal = (await ctx.db.get(proposalId))!;
    ...
    const runnerConfig = agent.executionContract.runnerConfig
      .filter((entry) => entry.key !== "artifactVersion")
      .concat({ key: "artifactVersion", value: A7_V9_RELEASE_SPEC.version.version });
    await ctx.db.patch(agent._id, {
      invocation: { type: "runtime", configRef: "server-owned:biocraft-singleshot-v9" },
      executionContract: { ...agent.executionContract, runnerConfig },
    });
    return {
      agentId: agent._id,
      ...
      proposalId: proposal._id,
      proposalStatus: proposal.status,
      releaseManifestDigest: A7_V9_RELEASE_MANIFEST_DIGEST,
      artifactDigest: A7_V9_RELEASE_SPEC.version.artifact.declaredDigest,
      nextRequiredAction: "Approve the returned proposal through reviews.approve to release v9.",
    };
  },
});
...
export const executeApprovedA10V3Release = mutation({
  args: { releaseManifestDigest: v.string() },
  handler: async (ctx, args) => {
    const actor = await requireApprover(ctx);
    if (args.releaseManifestDigest !== A10_V3_RELEASE_MANIFEST_DIGEST) {
      throw new ConvexError({
        code: "RELEASE_MANIFEST_NOT_APPROVED",
        status: 403,
        message: "A10 v3 release manifest is altered or not approved",
      });
    }
    const now = Date.now();
    const matches = await ctx.db
      ...
      .collect();
    if (matches.length !== 1) {
      throw new Error("A10 v3 release requires exactly one existing A10 agent");
    }
    const agent = matches[0];
    const prior = await oneVersion(ctx, agent._id, A10_V3_RELEASE_SPEC.priorVersion);
    if (!prior || prior.artifact?.declaredDigest !== A10_V3_RELEASE_SPEC.priorArtifactSha256) {
      throw new Error("A10 v3 release requires the exact governed gapfill-v2 artifact as its base");
    }
    const current = approvedVersionOf(agent as unknown as Record<string, unknown>);
    let candidate = await oneVersion(ctx, agent._id, A10_V3_RELEASE_SPEC.version.version);
    if (current !== prior._id && current !== candidate?._id) {
      throw new Error("A10 v3 release requires gapfill-v2 or its exact v3 successor to be current");
    }
    if (candidate) {
      assertExact(
        "A10 v3",
        { state: candidate.state, artifact: candidate.artifact, basedOnVersionId: candidate.basedOnVersionId },
        {
          state: A10_V3_RELEASE_SPEC.version.state,
          artifact: A10_V3_RELEASE_SPEC.version.artifact,
          basedOnVersionId: prior._id,
        },
      );
    } else {
      const candidateId = await ctx.db.insert("agentVersions", {
        agentId: agent._id,
        version: A10_V3_RELEASE_SPEC.version.version,
        state: A10_V3_RELEASE_SPEC.version.state,
        basedOnVersionId: prior._id,
        artifact: A10_V3_RELEASE_SPEC.version.artifact,
        createdBy: actor,
    ...
    if (proposal) {
      assertExact(
        ...
        },
        {
          agentId: agent._id,
          priorApprovedVersionId: prior._id,
          summary: A10_V3_RELEASE_SPEC.proposalSummary,
        },
      );
    } else {
      const proposalId = await ctx.db.insert("proposals", {
    ...
    const runnerConfig = agent.executionContract.runnerConfig
      .filter((entry) => entry.key !== "artifactVersion")
      .concat({ key: "artifactVersion", value: A10_V3_RELEASE_SPEC.version.version });
    await ctx.db.patch(agent._id, {
      invocation: { type: "runtime", configRef: "server-owned:biocraft-gapfill-v3" },
      executionContract: { ...agent.executionContract, runnerConfig },
    });
    return {
      agentId: agent._id,
      displayId: A10_V3_RELEASE_SPEC.agentDisplayId,
      versionId: candidate._id,
      proposalId: proposal._id,
      proposalStatus: proposal.status,
      releaseManifestDigest: A10_V3_RELEASE_MANIFEST_DIGEST,
